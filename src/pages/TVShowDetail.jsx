...
export default function TVShowDetail() {
  ...
  useEffect(() => {
    const fetchTVShowAndRating = async () => {
      const url = `https://api.themoviedb.org/3/tv/${id}?api_key=${import.meta.env.VITE_TMDB_API_KEY}`;
      ...
      if (currentUser) {
        ...
        if (watchedSnap.exists() && watchedSnap.data()[`tv_${id}`]) {
          savedRating = watchedSnap.data()[`tv_${id}`].rating || 0;
          setComment(watchedSnap.data()[`tv_${id}`].comment || "");
          setIsWatched(true);
        } else if (watchlistSnap.exists() && watchlistSnap.data()[`tv_${id}`]) {
          savedRating = watchlistSnap.data()[`tv_${id}`].rating || 0;
          setComment(watchlistSnap.data()[`tv_${id}`].comment || "");
  ...
  useEffect(() => {
    const fetchStreamingProviders = async () => {
      ...
      try {
        const url = `https://api.themoviedb.org/3/tv/${id}/watch/providers?api_key=${import.meta.env.VITE_TMDB_API_KEY}`;
        const res = await fetch(url);
        const data = await res.json();
  ...
  const handleRating = async (star) => {
    ...
    if (!currentUser) {
      toast.error("You must be logged in to rate TV shows!");
      return;
    }
    ...
    await Promise.all([
      setDoc(watchlistRef, { [`tv_${tvShow.id}`]: tvShowData }, { merge: true }),
      setDoc(watchedRef, { [`tv_${tvShow.id}`]: tvShowData }, { merge: true }),
    ]);
  ...
  const saveToWatchlist = async () => {
    if (!currentUser) {
      toast.error("You must be logged in to save TV shows!");
      return;
    }
    ...
    try {
      ...
      toast.success(`${tvShow.name} saved to your watchlist!`);
      setIsOnWatchlist(true);
    } catch (error) {
    ...
    <Layout>
      <div className="max-w-3xl mx-auto p-6">
        ...
        <div className="text-gray-700 dark:text-gray-200 space-y-2 mb-6 text-left">
          ...
          <p><strong>TMDB Rating:</strong> {tvShow.vote_average}</p>
